...
export async function GET() {
  try {
    ...
    try {
      ...
      if (error) {
        logger.warn("Table mock_interviews not available or error:", error.message);
        return NextResponse.json({ success: true, data: [], dbError: error.message, fallback: true });
      }

      return NextResponse.json({ success: true, data });
    } catch (dbErr: any) {
      logger.warn("Graceful db catch for interviews history GET:", dbErr.message);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (error) {
        logger.warn("Supabase delete failed on mock_interviews:", error.message);
        return NextResponse.json({ success: false, error: error.message });
      }

      return NextResponse.json({ success: true });
    } catch (dbErr: any) {
      logger.warn("Supabase delete exception caught on mock_interviews:", dbErr.message);
