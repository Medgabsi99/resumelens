...
import {
  GitCompare, ArrowRight, Check, X, Star,
  AlertTriangle, Play, Sparkles, FileText, Briefcase
} from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="font-display text-2xl font-bold text-ink leading-tight">A/B Resume Testing</h1>
              <p className="text-sm text-ink-muted mt-0.5">
                Compare two resume versions side-by-side to optimize scores and keyword alignment before applying.
              </p>
            </div>
          </div>
          ...
          <div className="flex bg-paper border border-border rounded-xl p-0.5 self-start">
            <button
              onClick={() => { setMode("saved"); setSelectedIdA(""); setSelectedIdB(""); }}
        ...
        {mode === "saved" && (
          <SpotlightCard className="glass-card bg-paper-card border border-border rounded-2xl p-6">
            <h3 className="font-display text-base font-bold text-ink mb-4">Select Reviews to Compare</h3>
            {dataLoading ? (
              <Skeleton h={80} />
            ) : analyses.length < 2 ? (
              <div className="text-center py-8 border border-dashed border-border rounded-xl bg-paper-warm/20 flex flex-col items-center">
                ...
                <p className="text-xs text-ink-muted mb-4 max-w-xs">You need at least 2 historical resume analyses to compare them here.</p>
                <Link href="/" className="btn-gradient px-4 py-2 rounded-xl text-xs font-semibold no-underline text-white">Analyze Resume</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-mono uppercase tracking-wider text-ink-faint">Resume A (Baseline)</label>
                  <select
                    value={selectedIdA}
                  ...
                  >
                    ...
                    {analyses.filter(a => a.id !== selectedIdB).map((a) => (
                      <option key={a.id} value={a.id}>
                        {a.target_role || "General Review"} ({new Date(a.created_at).toLocaleDateString()}) - Score: {a.score}%
                      </option>
                    ))}
                  </select>
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-mono uppercase tracking-wider text-ink-faint">Resume B (Comparison)</label>
                  <select
                    value={selectedIdB}
                  ...
                  >
                    ...
                    {analyses.filter(a => a.id !== selectedIdA).map((a) => (
                      <option key={a.id} value={a.id}>
                        {a.target_role || "General Review"} ({new Date(a.created_at).toLocaleDateString()}) - Score: {a.score}%
                      </option>
                    ))}
        ...
        {mode === "test-jd" && (
          <SpotlightCard className="glass-card bg-paper-card border border-border rounded-2xl p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-mono uppercase tracking-wider text-ink-faint">Target Role Title (Optional)</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-mono uppercase tracking-wider text-ink-faint">Target Job Description</label>
                <textarea
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] font-mono uppercase tracking-wider text-ink-faint font-semibold">Resume Text A</label>
                  {analyses.length > 0 && (
                    <select
                      ...
                      defaultValue=""
                    >
                      <option value="" disabled>Load from past review...</option>
                      {analyses.map(a => (
                        <option key={a.id} value={a.id}>{a.target_role || "Review"} ({new Date(a.created_at).toLocaleDateString()})</option>
                      ))}
                    </select>
                  )}
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] font-mono uppercase tracking-wider text-ink-faint font-semibold">Resume Text B</label>
                  {analyses.length > 0 && (
                    <select
            ...
            >
              {isTesting ? (
                <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Analyzing both versions against job description...</>
              ) : (
        ...
        <AnimatePresence mode="wait">
          {resultA && resultB ? (
            ...
            >
                ...
                <div className="flex items-center gap-4 bg-emerald-400/8 border border-emerald-400/20 rounded-2xl px-6 py-4">
                  ...
                  <div>
                    <h4 className="font-semibold text-sm text-ink flex items-center gap-1.5">
                      Recommendation: Use <span className="text-emerald-400 font-bold">{recommendation.winner}</span>
                    </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <SpotlightCard className="glass-card bg-paper-card border border-border rounded-2xl p-6 space-y-6 relative overflow-hidden">
                  ...
                  {/* Card Header */}
                  <div>
                    <span className="text-[9px] font-mono uppercase tracking-widest text-ink-faint">{infoA.title}</span>
                    <h3 className="font-display text-lg font-bold text-ink mt-0.5 truncate">{infoA.role}</h3>
                  </div>
                  ...
                  <div className="flex items-center gap-4 bg-paper border border-border p-4 rounded-xl">
                    ...
                    <div>
                      <div className="text-[10px] font-mono text-ink-faint uppercase tracking-wider">ATS MATCH SCORE</div>
                      <div className="text-xs text-ink-muted leading-relaxed mt-0.5">{resultA.summary}</div>
                    </div>
                  </div>
                  ...
                  {resultA.ats_breakdown && (
                    <div className="space-y-2.5">
                      <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">ATS Parameters</h4>
                      {[
                        ...
                        { label: "Readability & Layout", value: resultA.ats_breakdown.readability },
                      ].map((item) => (
                        <div key={item.label}>
                  ...
                  {/* Keywords matched */}
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Matched Keywords ({resultA.keywords_matched?.length ?? 0})</h4>
                    <div className="flex flex-wrap gap-1.5">
                      {resultA.keywords_matched?.slice(0, 15).map(k => (
                        <span key={k} className="bg-emerald-400/10 text-emerald-400 border border-emerald-400/20 px-2 py-0.5 rounded-lg text-xs font-semibold flex items-center gap-1">
                          <Check size={10} /> {k}
                        </span>
                      ))}
                    </div>
                  ...
                  {/* Keywords missing */}
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Missing Keywords ({resultA.keywords_missing?.length ?? 0})</h4>
                    <div className="flex flex-wrap gap-1.5">
                      {resultA.keywords_missing?.slice(0, 15).map(k => (
                        <span key={k} className="bg-rose-400/10 text-rose-400 border border-rose-400/20 px-2 py-0.5 rounded-lg text-xs font-semibold flex items-center gap-1">
                          <X size={10} /> {k}
                        </span>
                      ))}
                    </div>
                  ...
                  {/* Strengths */}
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Strengths</h4>
                    <ul className="text-xs text-ink-muted space-y-1 pl-4 list-disc">
                      {resultA.strengths.slice(0, 4).map((s, i) => <li key={i}>{s}</li>)}
                    </ul>
                  </div>

                  {/* Weaknesses */}
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Areas to Improve</h4>
                    <ul className="text-xs text-ink-muted space-y-1 pl-4 list-disc">
                      {resultA.weaknesses.slice(0, 4).map((w, i) => <li key={i}>{w}</li>)}
                    </ul>
                  </div>
                ...
                <SpotlightCard className="glass-card bg-paper-card border border-border rounded-2xl p-6 space-y-6 relative overflow-hidden">
                  ...
                  {/* Card Header */}
                  <div>
                    <span className="text-[9px] font-mono uppercase tracking-widest text-ink-faint">{infoB.title}</span>
                    <h3 className="font-display text-lg font-bold text-ink mt-0.5 truncate">{infoB.role}</h3>
                  </div>
                  ...
                  <div className="flex items-center gap-4 bg-paper border border-border p-4 rounded-xl">
                    ...
                    <div>
                      <div className="text-[10px] font-mono text-ink-faint uppercase tracking-wider">ATS MATCH SCORE</div>
                      <div className="text-xs text-ink-muted leading-relaxed mt-0.5">{resultB.summary}</div>
                    </div>
                  </div>
                  ...
                  {resultB.ats_breakdown && (
                    <div className="space-y-2.5">
                      ...
                      {[
                        ...
                        { label: "Readability & Layout", value: resultB.ats_breakdown.readability },
                      ].map((item) => (
                        <div key={item.label}>
                          <div className="flex justify-between text-xs mb-1">
                            <span className="text-ink-muted">{item.label}</span>
                  ...
                  {/* Keywords matched */}
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Matched Keywords ({resultB.keywords_matched?.length ?? 0})</h4>
                    <div className="flex flex-wrap gap-1.5">
                  ...
                  {/* Keywords missing */}
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Missing Keywords ({resultB.keywords_missing?.length ?? 0})</h4>
                    <div className="flex flex-wrap gap-1.5">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Strengths</h4>
                    <ul className="text-xs text-ink-muted space-y-1 pl-4 list-disc">
                      {resultB.strengths.slice(0, 4).map((s, i) => <li key={i}>{s}</li>)}
                    </ul>
                  </div>
                  ...
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono uppercase tracking-widest text-ink-faint">Areas to Improve</h4>
                    <ul className="text-xs text-ink-muted space-y-1 pl-4 list-disc">
                      {resultB.weaknesses.slice(0, 4).map((w, i) => <li key={i}>{w}</li>)}
                    </ul>
                  </div>
            ...
            <div className="text-center py-16 border border-dashed border-border rounded-xl bg-paper-warm/20 flex flex-col items-center">
              ...
              <p className="font-semibold text-ink text-sm mb-1">Select resumes to begin analysis</p>
