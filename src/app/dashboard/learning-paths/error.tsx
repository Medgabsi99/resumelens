...
export default function LearningPathsErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Learning Paths local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <h2 className="font-display text-2xl font-bold tracking-tight text-ink mb-3">
        Learning Path Workspace Error
      </h2>

      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        We encountered a problem assembling your personalized learning milestones and portfolio project scope. Check the console logs for network API details.
      </p>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 justify-center">
        ...
        >
          <RefreshCw size={14} /> Rebuild Path
        </button>
