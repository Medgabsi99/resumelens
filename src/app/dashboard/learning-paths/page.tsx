  ...
  const loadData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (local) {
          localHistory = JSON.parse(local).map((item: Partial<SavedPathItem>) => ({
            ...item,
            isLocal: true,
          }));
        }
          ...
          {!activePathData && !generating && (
            <div className="space-y-1.5">
              ...
              <p className="text-ink-muted text-sm max-w-3xl">
                Bridge technical gaps identified between your resume and a target job. Generate
                custom week-by-week learning curriculums and structured portfolio projects to
          ...
          {generating && (
            <div className="bg-paper-card border border-border rounded-2xl p-12 text-center flex flex-col items-center justify-center gap-4 shadow-lg min-h-[350px]">
              ...
              <p className="text-xs text-ink-muted">
                Leveraging gemini-2.5 models to customize architecture configurations and Weekly
                timelines
              </p>
            </div>
          )}
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-paper-card border border-border rounded-2xl p-6 space-y-5 shadow-lg">
                  ...
                  <button
                    ...
                    disabled={
                      !selectedResumeId || !roleTitle || !companyName || !jobDescription.trim()
                    }
                  ...
                  {loading ? (
                    ...
                    <div className="text-center text-xs text-ink-muted py-8">
                      ...
                      <p className="text-[10px] text-ink-faint mt-1">
                        Configure your target credentials above to begin tracking!
                      </p>
                    </div>
                  ) : (
                      ...
                      {history.map((item) => (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <h4
                                className="font-bold text-sm text-ink truncate max-w-[140px]"
                            ...
                            {item.isLocal && (
                              <span
                                className="bg-slate-500/10 text-slate-400 text-[8px] font-bold px-1.5 py-0.5 rounded border border-slate-500/20"
                          ...
                          <div className="flex flex-wrap gap-1">
                            {item.missing_skills.slice(0, 3).map((sk) => (
                              <span
                                key={sk}
                                className="bg-emerald-500/5 text-emerald-400 text-[9px] font-semibold border border-emerald-500/10 px-1.5 py-0.2 rounded"
                              >
                                {sk}
                              </span>
                            ))}
