...
import { Wand2, RefreshCw, LayoutDashboard } from "lucide-react";
...
export default function TailorErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Auto-Tailoring Workspace local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        The auto-tailor workspace encountered a layout or loading issue. Try resetting the workspace or refreshing the base resume.
      </p>
