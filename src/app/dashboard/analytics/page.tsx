...
import {
  BarChart2, TrendingUp, Target, Briefcase,
  ArrowRight, Star, AlertTriangle, ChevronDown, ChevronUp
} from "lucide-react";

// ─── Chart 1 helpers: All-time score trend ────────────────────────────────────

const W = 520, H = 200;
const PAD = { top: 24, right: 30, bottom: 32, left: 36 };
const INNER_W = W - PAD.left - PAD.right;
const INNER_H = H - PAD.top - PAD.bottom;

interface TrendPoint { x: number; y: number; score: number; date: string; role: string; }
...
interface RoleStat { role: string; max: number; avg: number; count: number; }

function buildRoleStats(
  analyses: { score: number; target_role: string | null }[]
): RoleStat[] {
  const map = new Map<string, number[]>();
  for (const a of analyses) {
    const key = (a.target_role?.trim() || "General").toLowerCase()
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm text-ink-muted mt-0.5">
            Score progression, role performance, and application conversion — evidence you&apos;re improving.
          </p>
        </div>
      </motion.div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
            ...
            {chart1 && (
              <div className="flex gap-5">
                {[
                  { label: "Latest",  value: chart1.lastScore, suffix: "/100", color: "var(--accent)" },
                  { label: "Best",    value: Math.max(...trendPoints.map(p => p.score)), suffix: "/100", color: "#f59e0b" },
                  { label: "Δ vs first", value: chart1.delta, suffix: " pts", prefix: chart1.delta > 0 ? "+" : "", color: chart1.delta > 0 ? "#10b981" : chart1.delta < 0 ? "#ef4444" : "var(--ink-muted)" },
                ].map(s => (
                  <div key={s.label} className="flex flex-col">
                    <span className="text-[9px] font-mono uppercase tracking-widest text-ink-faint">{s.label}</span>
                    <span className="text-lg font-extrabold" style={{ color: s.color, fontFamily: "DM Serif Display, serif", lineHeight: 1.2 }}>
          ...
          {loading ? <Skeleton h={200} /> : chart1 ? (
            <div className="relative w-full">
              <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
                ...
                {chart1.gridY.map(v => (
                  <g key={v}>
                    <line x1={PAD.left} y1={chart1.yOf(v)} x2={W - PAD.right} y2={chart1.yOf(v)}
                      stroke="var(--border)" strokeWidth={1} strokeDasharray="4,4" />
                    <text x={PAD.left - 6} y={chart1.yOf(v) + 3} textAnchor="end"
                      fill="var(--ink-faint)" fontSize={9} fontFamily="DM Mono, monospace">{v}</text>
                  </g>
                ))}

                {/* Milestone lines */}
                {[{ v: 70, label: "ATS OK", color: "#f59e0b" }, { v: 85, label: "Strong", color: "#10b981" }]
                  .filter(m => m.v >= chart1.minS && m.v <= chart1.maxS)
                  .map(m => (
                    <g key={m.v}>
                      <line x1={PAD.left} y1={chart1.yOf(m.v)} x2={W - PAD.right} y2={chart1.yOf(m.v)}
                        stroke={m.color} strokeWidth={1} strokeOpacity={0.5} strokeDasharray="6,3" />
                      <text x={W - PAD.right + 4} y={chart1.yOf(m.v) + 3}
                        fill={m.color} fontSize={8} fontFamily="DM Mono, monospace" fontWeight={700} opacity={0.85}>
                        {m.label}
                      </text>
                    </g>
                  ))}
                ...
                {/* Rolling avg line */}
                <path d={chart1.avgPath} fill="none" stroke="var(--accent)" strokeWidth={1}
                  strokeOpacity={0.3} strokeDasharray="5,3" strokeLinecap="round" />

                {/* Area fill */}
                <path d={chart1.areaPath} fill="url(#trendFill)" />

                {/* Score line */}
                <path d={chart1.linePath} fill="none" stroke="var(--accent)" strokeWidth={2.5}
                  strokeLinecap="round" strokeLinejoin="round" />

                {/* Hover crosshair */}
                {hoveredPt !== null && chart1.mapped[hoveredPt] && (
                  <line x1={chart1.mapped[hoveredPt].cx} y1={PAD.top}
                    x2={chart1.mapped[hoveredPt].cx} y2={H - PAD.bottom}
                    stroke="var(--accent)" strokeOpacity={0.3} strokeWidth={1.5} strokeDasharray="2,2" />
                )}
                ...
                {chart1.mapped.map((pt, i) => {
                  ...
                  return (
                    <g key={i}>
                      {isBest && <circle cx={pt.cx} cy={pt.cy} r={11} fill="#f59e0b" opacity={0.15} />}
                      <circle cx={pt.cx} cy={pt.cy} r={isHov ? 6 : isBest ? 5.5 : 4}
                        fill={isHov ? "var(--accent)" : isBest ? "#f59e0b" : "var(--paper-card)"}
                        stroke={isBest ? "#f59e0b" : "var(--accent)"} strokeWidth={2}
                        style={{ cursor: "pointer" }}
                        onMouseEnter={() => setHoveredPt(i)}
                        onMouseLeave={() => setHoveredPt(null)} />
                      {isBest && !isHov && (
                        <text x={pt.cx} y={pt.cy - 10} textAnchor="middle"
                          fontSize={9} fill="#f59e0b" fontWeight={700} fontFamily="DM Mono, monospace">
                          ★ {pt.score}
                        </text>
                      )}
                    </g>
                ...
                {[0, Math.floor(chart1.mapped.length / 2), chart1.mapped.length - 1]
                  ...
                  .map(i => (
                    <text key={i} x={chart1.mapped[i].cx} y={H - 10} textAnchor="middle"
                      fill="var(--ink-faint)" fontSize={9} fontFamily="DM Mono, monospace">
              ...
              {/* Hover tooltip */}
              {hoveredPt !== null && chart1.mapped[hoveredPt] && (
                <div className="absolute top-2 glass-card bg-paper-card border border-accent-border px-3 py-2 rounded-xl pointer-events-none shadow-lg z-20"
                  style={{
                    left: chart1.mapped[hoveredPt].cx / W > 0.5 ? 8 : "auto",
                    right: chart1.mapped[hoveredPt].cx / W > 0.5 ? "auto" : 8,
                  }}>
                  <div className="text-[10px] font-mono text-accent uppercase tracking-wider font-semibold">{chart1.mapped[hoveredPt].date}</div>
                  <div className="text-xs font-semibold text-ink-muted truncate max-w-[140px]">{chart1.mapped[hoveredPt].role}</div>
                  <div className="text-lg font-bold text-ink mt-0.5">
                    Score: <span className="text-accent">{chart1.mapped[hoveredPt].score}</span>/100
              ...
              <div className="flex items-center gap-3 mt-2 text-[10px] font-mono text-ink-faint">
                <span className="flex items-center gap-1.5">
                  <svg width="20" height="6"><line x1="0" y1="3" x2="20" y2="3" stroke="var(--accent)" strokeWidth="2.5" strokeLinecap="round" /></svg>
                  Score
                </span>
                <span className="flex items-center gap-1.5">
                  <svg width="20" height="6"><line x1="0" y1="3" x2="20" y2="3" stroke="var(--accent)" strokeWidth="1.5" strokeDasharray="5,3" strokeOpacity="0.5" /></svg>
                  4-pt rolling avg
                </span>
              </div>
            </div>
            ...
            <div className="text-center py-14 border border-dashed border-border rounded-xl bg-paper-warm/20 flex flex-col items-center">
              ...
              <p className="text-xs text-ink-muted mb-4 max-w-xs">Run your first resume analysis to start tracking your score improvement over time.</p>
              <Link href="/" className="text-xs font-semibold text-accent border border-accent-border hover:bg-accent-bg px-4 py-2 rounded-lg no-underline transition-all flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <SpotlightCard className="glass-card bg-paper-card border border-border rounded-2xl p-6 h-full">
            ...
            <p className="text-xs text-ink-muted mb-5">Highest ATS score achieved per target job type</p>

            {loading ? <Skeleton h={220} /> : roleStats.length > 0 ? (
              <div className="space-y-3">
                {visibleRoles.map((r, i) => (
                  <div key={r.role}>
                    <div className="flex justify-between items-baseline text-xs mb-1">
                      <span className="font-semibold text-ink truncate max-w-[55%]" title={r.role}>{r.role}</span>
                      <span className="text-ink-muted font-mono flex items-center gap-2 shrink-0">
                        ...
                        <span className="font-bold text-ink">{r.max}<span className="text-ink-faint">/100</span></span>
                    ...
                    <div className="relative h-2.5 bg-paper-warm rounded-full overflow-hidden border border-border">
                      ...
                      <div className="absolute h-full rounded-full transition-all duration-700"
                        style={{ width: `${r.avg}%`, background: "var(--accent)", opacity: 0.25 }} />
                      {/* Max bar */}
                      <motion.div
                        ...
                        style={{ background: i === 0 ? "#f59e0b" : "var(--accent)", boxShadow: `0 0 8px ${i === 0 ? "#f59e0b44" : "var(--accent)33"}` }}
                      />
                    </div>
                  </div>
                ))}
                ...
                {roleStats.length > 8 && (
                  ...
                  >
                    {showAllRoles ? <><ChevronUp size={12} /> Show less</> : <><ChevronDown size={12} /> Show {roleStats.length - 8} more roles</>}
                  </button>
                )}
              </div>
            ) : (
              <div className="text-center py-12 border border-dashed border-border rounded-xl bg-paper-warm/20 flex flex-col items-center">
                ...
                <p className="text-xs text-ink-muted mb-3 max-w-xs">Add a target role when running your next analysis to see per-role insights.</p>
                <Link href="/" className="text-xs font-semibold text-accent border border-accent-border hover:bg-accent-bg px-3 py-1.5 rounded-lg no-underline transition-all">
                  Analyze Now
                </Link>
              </div>
            )}
        ...
        >
          <SpotlightCard className="glass-card bg-paper-card border border-border rounded-2xl p-6 h-full">
            ...
            <p className="text-xs text-ink-muted mb-4">How well your applications are converting through each stage</p>
            ...
            {!loading && applications.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-5">
                {[
                  ...
                  const v = kpi.value;
                  const color = v === null ? "var(--ink-muted)"
                    : v >= kpi.threshold[1] ? "#10b981"
                    : v >= kpi.threshold[0] ? "#f59e0b"
                    : "#ef4444";
                  return (
                    <div key={kpi.label} className="flex flex-col items-center justify-center bg-paper border border-border rounded-xl px-4 py-3 min-w-[90px]">
                      <span className="text-xl font-extrabold" style={{ color, fontFamily: "DM Serif Display, serif" }}>
                        {v === null ? "—" : `${v}%`}
                      </span>
                      <span className="text-[9px] font-mono text-ink-faint text-center leading-tight mt-0.5">{kpi.label}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {/* Funnel bars */}
            {loading ? <Skeleton h={150} /> : applications.length > 0 ? (
              <div className="space-y-2.5">
                    ...
                    <div key={idx} className="group">
                      ...
                      <div className="h-2.5 w-full bg-paper-warm rounded-full overflow-hidden border border-border">
                        <motion.div
                          ...
                          initial={{ width: 0 }}
                          animate={{ width: `${pct}%` }}
                          transition={{ delay: 0.2 + idx * 0.07, duration: 0.55, ease: "easeOut" }}
                          style={{ background: stage.color, boxShadow: `0 0 10px ${stage.color}22` }}
                        />
                      </div>
                    </div>
                  );
                ...
                {kpis.overall !== null && (
                  <div className="mt-4 flex items-center gap-2 text-xs font-mono">
                    {kpis.overall >= 30 ? (
                      <><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block" /><span className="text-emerald-400 font-bold">Strong pipeline</span><span className="text-ink-muted">— keep up the momentum</span></>
                    ) : kpis.overall >= 10 ? (
                      <><AlertTriangle size={12} className="text-amber-400" /><span className="text-amber-400 font-bold">Pipeline building</span><span className="text-ink-muted">— focus on applications</span></>
                    ) : (
                      <><AlertTriangle size={12} className="text-red-400" /><span className="text-red-400 font-bold">Low conversion</span><span className="text-ink-muted">— tailor resume to JDs</span></>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="text-center py-10 border border-dashed border-border rounded-xl bg-paper-warm/20 flex flex-col items-center">
                ...
                <p className="text-xs text-ink-muted mb-3 max-w-xs">Track your job applications to see conversion rates here.</p>
                <Link href="/dashboard/applications" className="text-xs font-semibold text-accent border border-accent-border hover:bg-accent-bg px-3 py-1.5 rounded-lg no-underline transition-all flex items-center gap-1.5">
        ...
        >
          ...
          <div>
            <p className="font-semibold text-sm text-ink">
              ...
              <span className="text-emerald-400 font-bold">+{chart1.delta} pts</span>{" "}
              across {analyses.length} analyses.
            </p>
            <p className="text-xs text-ink-muted mt-0.5">
              That&apos;s real, measurable progress. Keep iterating — most ATS thresholds are 70+.
            </p>
          </div>
        </motion.div>
      )}
