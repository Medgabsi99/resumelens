...
export default function AnalyticsErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Analytics local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        The analytics dashboard failed to load. This may be a temporary data issue — please refresh.
      </p>
      {error.message && (
