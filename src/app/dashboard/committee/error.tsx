...
import { Users, RefreshCw, LayoutDashboard } from "lucide-react";
...
export default function CommitteeErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Hiring Committee local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <h2 className="font-display text-2xl font-bold tracking-tight text-ink mb-3">
        Hiring Panel Simulator Error
      </h2>

      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        The hiring committee simulation encountered an error. This can happen with long transcripts, model timeout limits, or network glitches during panel debrief rendering.
      </p>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 justify-center">
        ...
        >
          <RefreshCw size={14} /> Restart Panel
        </button>
