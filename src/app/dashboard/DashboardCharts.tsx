        ...
        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-ink-muted">
              ATS score improvement over your resume revisions
            </p>
          </div>
          {/* Period selector */}
              ...
              <div key={s.label} style={{ display: "flex", flexDirection: "column" }}>
                ...
                <span
                  style={{
                    fontSize: 16,
                    fontWeight: 800,
                    color: (s as { color?: string }).color ?? "var(--ink)",
          ...
          {loading ? (
            ...
            <div className="w-full relative">
                ...
                {scoreChartData.milestones.map((m) => {
                  ...
                  return (
                    <g key={m.value}>
                      ...
                      />
                      <text
                        x={scoreChartData.svgW - scoreChartData.padding.right + 6}
                        y={y + 3}
                        fill={m.color}
                        fontSize={8.5}
                        fontFamily="DM Mono, monospace"
                  ...
                  return (
                    <g key={idx}>
                      ...
                      <circle
                        ...
                        fill={
                          isHovered ? "var(--accent)" : isBest ? "#f59e0b" : "var(--paper-card)"
                        }
              ...
              {hoveredPoint !== null && scoreChartData.points[hoveredPoint] && (
                <div
                  ...
                  style={{
                    right:
                      scoreChartData.points[hoveredPoint].x < scoreChartData.svgW / 2
                        ? "8px"
                        : "auto",
                    left:
                      scoreChartData.points[hoveredPoint].x < scoreChartData.svgW / 2
                ...
                >
                  ...
                  <div className="text-xl font-bold text-ink mt-0.5">
                    Score:{" "}
                    <span className="text-accent">{scoreChartData.points[hoveredPoint].score}</span>
                    /100
                    {hoveredPoint === scoreChartData.bestIdx && (
                      <span className="ml-1.5 text-[10px] font-bold text-amber-500 bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded-md inline-flex items-center gap-0.5">
                        <Star size={9} fill="currentColor" /> Personal Best
                      </span>
                    )}
                  </div>
                </div>
            ...
            <div className="text-center p-6 border border-dashed border-border rounded-xl w-full flex flex-col items-center justify-center bg-paper-warm/20">
              ...
              <p className="text-xs text-ink-muted max-w-[250px] mb-4">
                Upload and analyze multiple versions of your resume to see your scores track over
                time.
              </p>
              <Link
                href="/"
      ...
      <SpotlightCard className="glass-card bg-paper-card p-6 rounded-2xl border border-border flex flex-col justify-between min-h-[300px]">
        <div>
          ...
          <p className="text-xs text-ink-muted mb-4">
            Status of active job search tracker opportunities
          </p>
        </div>
        <div className="flex-1 flex flex-col justify-center gap-3">
          {loading ? (
            <div className="text-ink-muted text-sm font-mono flex items-center justify-center gap-2">
              <span className="w-4 h-4 border-2 border-accent/20 border-t-accent rounded-full animate-spin" />{" "}
              Loading pipeline...
            </div>
            ...
            <div className="text-center p-6 border border-dashed border-border rounded-xl w-full flex flex-col items-center justify-center bg-paper-warm/20">
              ...
              <p className="text-xs text-ink-muted max-w-[250px] mb-4">
                Use our job tracker to manage your applications, screening schedules, and interview
                progress.
              </p>
              <Link
