  ...
  const loadData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (local) {
          localHistory = JSON.parse(local).map((item: Partial<SavedNegotiationItem>) => ({
            ...item,
            isLocal: true,
          }));
        }
  ...
  const handleDeleteHistoryItem = async (item: ScorecardItem) => {
    ...
    if (item.isLocal) {
      ...
      try {
        ...
        toastError(
          (err as Error).message || "Failed to delete entry from database.",
          "Delete Failed"
        );
      }
    }
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-ink-muted text-sm">
                Practice compensation negotiation scenarios with our interactive AI Recruiter.
                Highlight your resume achievements to maximize your offers.
              </p>
            </div>
          </div>
              ...
              <div className="bg-paper-card border border-border rounded-2xl p-6 space-y-5 shadow-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1.5 md:col-span-2">
                    ...
                    <select
                      ...
                      className="w-full bg-paper border border-border rounded-xl px-4 py-2.5 text-sm text-ink outline-none focus:border-accent transition"
                    >
                      <option value="initial_offer">
                        Initial Job Offer (Candidate vs Recruiter)
                      </option>
                      <option value="performance_review">
                        Annual Performance Raise (Employee vs Manager)
                      </option>
                      <option value="competing_offer">
                        Counter competing offer match (Competing leverage)
                      </option>
                    </select>
                  </div>
                </div>
                ...
                <div className="border-t border-border pt-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Base Salary */}
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs text-ink-faint font-medium">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs text-ink-faint font-medium">
                        Equity/RSUs Value (USD total value)
                      </label>
                      <input
                        type="number"
                        value={equity}
              ...
              <div className="bg-paper-card border border-border rounded-2xl p-6 space-y-4 shadow-lg h-full">
                ...
                {loading ? (
                  ...
                  <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                    {history.map((item) => {
                      const finalTotal =
                        item.final_offer.base + item.final_offer.equity + item.final_offer.signOn;
                      const initialTotal =
                        item.initial_offer.base +
                        item.initial_offer.equity +
                        item.initial_offer.signOn;
              ...
              <div className="w-full max-w-2xl bg-paper border border-border rounded-2xl shadow-2xl p-6 md:p-8 space-y-6 max-h-[85vh] overflow-y-auto animate-scale-up text-slate-100 font-sans">
                ...
                <div className="text-center border-b border-border pb-5 relative">
                  ...
                  <h3 className="font-display text-2xl font-bold text-ink">
                    Saved Scorecard Result
                  </h3>
                  <p className="text-ink-muted text-sm mt-1">
                    {selectedHistoryItem.role_title} at{" "}
                    <span className="font-semibold">{selectedHistoryItem.company_name}</span>
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-paper-card border border-border p-4 rounded-xl flex flex-col items-center justify-center">
                    ...
                    <span className="text-3xl font-extrabold text-accent mt-1">
                ...
                {/* Tactics */}
                {selectedHistoryItem.feedback?.tacticsUsed &&
                  selectedHistoryItem.feedback.tacticsUsed.length > 0 && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <h4 className="font-display text-xs font-bold text-ink-muted uppercase tracking-wider text-emerald-400">
                      Strengths
                    </h4>
                  ...
                  <div className="space-y-2">
                    <h4 className="font-display text-xs font-bold text-ink-muted uppercase tracking-wider text-rose-400">
                      Areas to Improve
                    </h4>
                    <ul className="space-y-1.5 text-xs text-ink-muted list-disc pl-4">
                ...
                {selectedHistoryItem.feedback?.transcript &&
                  selectedHistoryItem.feedback.transcript.length > 0 && (
                    <div className="border-t border-border pt-4 mt-2">
                      <details className="group">
                        ...
                        <div className="mt-4 space-y-4 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                          {selectedHistoryItem.feedback.transcript.map(
