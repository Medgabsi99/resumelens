...
import { Briefcase, RefreshCw, LayoutDashboard } from "lucide-react";
...
export default function ApplicationsErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Applications tracker local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <h2 className="font-display text-2xl font-bold tracking-tight text-ink mb-3">
        Applications Tracker Error
      </h2>

      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        The application tracking dashboard failed to load. This might be due to database synchronization lag or network timeout fetching profiles.
      </p>
