...
import { LayoutTemplate, RefreshCw, LayoutDashboard } from "lucide-react";
...
export default function BuilderErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Resume Builder local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        The resume builder failed to load. Your saved data is safe — please refresh to continue
        editing.
      </p>
      {error.message && (
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 justify-center">
        ...
        >
          <RefreshCw size={14} /> Reload Builder
        </button>
        <Link
          href="/dashboard"
