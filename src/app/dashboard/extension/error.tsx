...
import { Puzzle, RefreshCw, LayoutDashboard } from "lucide-react";
...
export default function ExtensionErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Browser Extension local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <h2 className="font-display text-2xl font-bold tracking-tight text-ink mb-3">
        Extension Integration Error
      </h2>
      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        Unable to load extension settings. Please try again.
      </p>
      {error.message && (
