...
import { GitCompare, RefreshCw, LayoutDashboard } from "lucide-react";
...
export default function CompareErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    logger.error("Compare Resumes local boundary error:", error);
  }, [error]);
  ...
  return (
    <div className="relative z-10 glass-card bg-paper-card border border-border rounded-2xl p-8 max-w-xl mx-auto my-12 text-center shadow-lg">
      ...
      <p className="text-ink-muted text-sm mb-6 leading-relaxed max-w-md mx-auto">
        An error occurred while comparing resumes. Please retry or exit to dashboard.
      </p>
      {error.message && (
