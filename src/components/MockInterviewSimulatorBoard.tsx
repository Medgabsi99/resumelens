      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-[#2c2c38] px-6 py-4">
          <div className="flex items-center gap-2.5">
            <span className="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider flex items-center gap-1.5">
              <Mic size={11} />
              AI Interview Coach
            </span>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="text-ink-muted hover:text-ink transition cursor-pointer flex items-center justify-center"
            ...
            <button
              onClick={onClose}
              className="text-ink-muted hover:text-ink border border-border/50 hover:border-accent-border px-3 py-1 rounded-xl text-xs transition cursor-pointer flex items-center gap-1.5"
            >
              <X size={12} />
              <span>Close</span>
            </button>
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 overflow-hidden">
            ...
            <div className="lg:col-span-2 flex flex-col p-6 overflow-y-auto border-r border-[#2c2c38]/50 space-y-6">
              ...
              <div className="flex-1 flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  ...
                  {isSupportedSpeech && (
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {isRecording
                          ? <><Square size={11} /><span>Stop Recording</span></>
                          : <><Mic size={11} /><span>Speak Response</span></>}
                      </button>
                    </div>
                  )}
                </div>
                ...
                {showHint && (
                  <div className="p-4 bg-indigo-500/5 border border-indigo-500/20 rounded-xl text-xs text-indigo-300 leading-relaxed font-mono flex items-start gap-2">
                    <Lightbulb size={12} className="shrink-0 mt-0.5" />
                    <span><strong>Coach Tip:</strong> Try to explain the <em>Context/Situation</em> clearly first. Detail what <em>Actions</em> you took specifically using the key tools, and state the final metric/outcome (<em>Result</em>)!</span>
                  </div>
                )}

                {evalError && (
                  <div className="p-3.5 bg-rose-500/5 border border-rose-500/20 rounded-xl text-xs text-rose-400 flex items-center gap-2">
            ...
            <div className="p-6 bg-[#181822]/50 space-y-6 overflow-y-auto">
              ...
              <div className="bg-[#181822] border border-[#2c2c38] rounded-xl p-4 space-y-3.5 shadow-md">
                ...
                <div className="text-[11px] text-ink-muted bg-paper border border-border p-2 rounded-lg leading-relaxed flex items-center gap-1.5">
                  <Activity size={11} className="shrink-0" />
                  <span><strong>Status:</strong> {getPacingRating(estimatedWpm)}</span>
                </div>
              </div>
